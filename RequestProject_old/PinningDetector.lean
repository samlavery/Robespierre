...
/-- Full-set imbalance: legacy aggregate over all nontrivial zeros.
    Defined as a topological sum (`tsum`) over the subtype `↥NontrivialZeros`.
    Defaults to `0` if the sum does not converge (Lean/Mathlib convention for
    `tsum`). Conditional and not used. -/
...
/-- Detector kernel: off-line zeros exist but the full-set detector does not fire
    for any density parameter `x > 1`. This is the "kernel void" regime where
    the aggregate signal disappears despite off-line input. -/
...
/-- The cancellation / rigidity / pinning class: the set of off-line zeros
    that coexist with a perfectly cancelling aggregate configuration, i.e.
    the off-line zeros conspiring to hide in the detector kernel voids.
    ...
    exactly cancelled by the aggregate of all other zeros, so the configuration
    hides in the full-set detector's kernel voids. -/
def pinningClass : Set ℂ :=
  { ρ ∈ OffLineZeros | ∀ x : ℝ, 1 < x → fullSetImbalance x = 0 }

/-! ### 1e. No-`tsum` repair: direct witness detector

The unweighted aggregate detector above is kept as a legacy observable, but
the proof route used by `ProofA` no longer relies on summability of
`fullSetImbalance`. Instead we expose a direct detector event built from the
already-valid per-zero witness statement. -/

/-- Direct pinning-detector fire: some off-line zero is witnessed by the
    per-zero detector. This is the no-`tsum` replacement for the aggregate
    `fullSetDetectorEvent`. -/
def directDetectorEvent : Prop :=
  ∃ ρ : ℂ, ρ ∈ OffLineZeros ∧ ∀ x : ℝ, 1 < x → perZeroDetectorEvent x ρ

/-- Direct detector silence: no off-line witness exists. -/
def directDetectorSilent : Prop :=
  ¬ directDetectorEvent

/-- Primitive cancelling / pinning class for the repaired detector semantics:
    an off-line zero that would persist only under the global assumption of
    detector silence. Since the direct detector fires on every off-line zero,
    this class is forced to be empty unconditionally. -/
def primitivePinningClass : Set ℂ :=
  { ρ ∈ OffLineZeros | directDetectorSilent }


/-- Primitive detector kernel: off-line zeros exist but the direct detector is
    silent. This is the no-`tsum` replacement for `detectorKernel`. -/
def primitiveDetectorKernel : Prop :=
  (∃ s, s ∈ OffLineZeros) ∧ directDetectorSilent
...
/-! ### 2e. On-line balance: σ = 1/2 forces no imbalance
...
/-- **Bridge Lemma 1: Summability (legacy aggregate route).**
    The unweighted per-zero imbalances are summable over the nontrivial zero
    set. This bridge belongs only to the legacy `fullSetImbalance` observable;
    the repaired direct-witness route avoids this assumption entirely. -/
...
/-- **Bridge Lemma 3: Off-line alternative.**
...
def BridgeOffLineAlternative : Prop :=
  ∀ ρ : ℂ, ρ ∈ OffLineZeros →
    (∃ x : ℝ, 1 < x ∧ fullSetDetectorEvent x) ∨ ρ ∈ pinningClass

/-- Legacy alias for `BridgeOffLineAlternative`. -/
abbrev BridgeDetectorSensitivity : Prop := BridgeOffLineAlternative
...
/-- **Bridge Lemma 5: Pinning-fire contradiction.**
    The aggregate pinning detector cannot genuinely fire. This packages the
    detector-side contradiction used by `ProofA`; the statement belongs here
    because it is formulated purely in terms of `fullSetDetectorEvent`. -/
def BridgePinningFireContradiction : Prop :=
  (∃ x : ℝ, 1 < x ∧ fullSetDetectorEvent x) → False

/-- **Bridge Lemma 5'': Direct witness contradiction.**
    This is the no-`tsum` detector bridge used by `ProofA`: the repaired direct
    detector cannot genuinely fire. -/
def BridgePinningWitnessContradiction : Prop :=
  directDetectorEvent → False
...
/-! ### Theorem B₀: Direct witness detector semantics (unconditional)

This is the no-`tsum` replacement for the aggregate `fullSetImbalance` route:
an off-line zero exists iff the direct witness detector fires. -/

/-- Any off-line zero fires the direct witness detector. -/
theorem directDetectorEvent_of_offLine
    {ρ : ℂ} (hρ : ρ ∈ OffLineZeros) :
    directDetectorEvent := by
  exact ⟨ρ, hρ, offLine_perZero_detection ρ hρ⟩

/-- The direct witness detector fires exactly when some off-line zero exists. -/
theorem directDetectorEvent_iff_hasOffLine :
    directDetectorEvent ↔ ∃ s, s ∈ OffLineZeros := by
  refine ⟨?_, ?_⟩
  · rintro ⟨ρ, hρ, _⟩
    exact ⟨ρ, hρ⟩
  · rintro ⟨ρ, hρ⟩
    exact directDetectorEvent_of_offLine hρ

/-- Direct detector silence is equivalent to absence of off-line zeros. -/
theorem directDetectorSilent_iff_noOffLine :
    directDetectorSilent ↔ ∀ s, s ∉ OffLineZeros := by
  refine ⟨?_, ?_⟩
  · intro hsilent s hs
    exact hsilent (directDetectorEvent_of_offLine hs)
  · intro hnone hfire
    rcases directDetectorEvent_iff_hasOffLine.mp hfire with ⟨s, hs⟩
    exact hnone s hs

/-- The repaired cancelling class is empty: once detector fire is defined by a
    direct witness rather than an unweighted `tsum`, there is no kernel void
    left for an off-line zero to hide in. -/
theorem primitivePinningClass_empty :
    primitivePinningClass = ∅ := by
  ext ρ
  refine ⟨?_, fun h => False.elim (Set.notMem_empty ρ h)⟩
  intro hρ
  exact hρ.2 (directDetectorEvent_of_offLine hρ.1)

/-- The repaired detector kernel is impossible. -/
theorem primitiveDetectorKernel_false :
    ¬ primitiveDetectorKernel := by
  rintro ⟨⟨ρ, hρ⟩, hsilent⟩
  exact hsilent (directDetectorEvent_of_offLine hρ)

/-! ### Theorem B': Full-set fire semantics

If the aggregate detector fires at some density parameter `x > 1`, then there
must exist an off-line zero. This is the detector-side converse to the
all-on-line full-set balance theorem, and needs only summability. -/

/-- **Theorem B'**: detector fire implies existence of an off-line zero
    (conditional on `BridgeSummability`). -/
theorem fires_implies_hasOffLine
    (hS : BridgeSummability) {x : ℝ} (hx : 1 < x)
    (hfire : fullSetDetectorEvent x) :
    ∃ s, s ∈ OffLineZeros := by
  by_contra hnone
  push_neg at hnone
  apply hfire
  have hall : ∀ s, s ∈ NontrivialZeros → s.re = 1 / 2 := by
    intro s hs
    by_contra hne
    exact hnone s ⟨hs, hne⟩
  exact onLine_balance_fullSet hS hall x hx
...
-/
theorem offLine_alternative (hB : BridgeOffLineAlternative) :
    ∀ ρ : ℂ, ρ ∈ OffLineZeros →
...
/-! ### Theorem F: User-facing detector dichotomy

This packages the detector semantics in the user-facing branch language:
fire implies an off-line witness (conditional on summability), while silence
forces either no off-line zeros or universal pinning (unconditional). -/

/-- **Theorem F₁**: detector silence implies either no off-line zeros exist or
    every off-line zero is pinned. -/
theorem silent_implies_noOffLine_or_allPinned :
    (∀ x : ℝ, 1 < x → ¬ fullSetDetectorEvent x) →
      (∀ s, s ∉ OffLineZeros) ∨ (∀ s, s ∈ OffLineZeros → s ∈ pinningClass) := by
  intro hsilent
  by_cases h : ∀ s, s ∉ OffLineZeros
  · left; exact h
  · right
    intro s hs
    exact rigidity_nondetection hsilent s hs

/-- **Theorem F₂**: detector semantics in one package. -/
theorem detector_dichotomy
    (hS : BridgeSummability) :
    ((∃ x : ℝ, 1 < x ∧ fullSetDetectorEvent x) → ∃ s, s ∈ OffLineZeros) ∧
    ((∀ x : ℝ, 1 < x → ¬ fullSetDetectorEvent x) →
      (∀ s, s ∉ OffLineZeros) ∨ (∀ s, s ∈ OffLineZeros → s ∈ pinningClass)) := by
  refine ⟨?_, silent_implies_noOffLine_or_allPinned⟩
  intro hfire
  rcases hfire with ⟨x, hx, hxfire⟩
  exact fires_implies_hasOffLine hS hx hxfire
...
- Theorem F₁/F₂: user-facing detector semantics (`fire` / `silent`)
...
- Theorem B': full-set fire ⇒ some off-line zero exists (needs BridgeSummability)
- Theorem C: off-line ⇒ detector fires OR pinning class (needs BridgeOffLineAlternative)
...
- BridgeOffLineAlternative: off-line ⇒ aggregate detection OR pinning
- BridgeWellDefinedness: channel pairing extends to full-set sums
- BridgePinningFireContradiction: aggregate pinning fire is impossible
-/
