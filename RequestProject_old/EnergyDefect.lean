...
`∫₀^∞ (∫₀^∞ f(t) cos(γt) dt)² dγ = (π/2) ∫₀^∞ f(t)² dt`, and the γ-integrand
on the LHS is integrable. Derivable from full-line Plancherel via even
extension: if f̃(t) = f(|t|), then F̂̃(ξ) = 2 ∫₀^∞ f(t) cos(2πξt) dt; apply
Plancherel, unfold symmetry, change variables. The constant π/2 arises from
the unnormalized convention. The integrability side is a standard corollary
of the full-line Plancherel. -/
...
theorem averageEnergyDefect_eq_weighted_L2 (ψ : ℝ → ℝ) (β : ℝ)
  ...
  obtain ⟨hC_int, hC_val⟩ := halfLine_cosine_parseval
    (fun t => amplitudeDefectEnvelope β t * ψ t) hf
  obtain ⟨hS_int, hS_val⟩ := halfLine_sine_parseval
    (fun t => oddDefectEnvelope β t * ψ t) hg
