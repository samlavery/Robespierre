...
/-- Every-prime inverse Euler-harmonic minimum gives the forked PerC
detector-native vanishing statement.  This is the direct harmonic-minimum
bridge: equality in the AM-GM prime envelope forces left/right detector
agreement, hence zero two-cosh residue, hence zero localized detector prime
observable. -/
theorem detectorPrimeHarmonicObservable_zero_every_prime_of_inverse_prime_harmonic_minimum
    (h_min : ∀ ρ : ℂ, ρ ∈ ZD.NontrivialZeros →
      ∀ p : ℕ, Nat.Prime p →
        ZD.KleinForcer.amplitude (p : ℝ) ρ.re =
          2 / Real.sqrt (p : ℝ)) :
  ...
  intro ρ hρ p hp
  have hres :
      ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicResidue
        ρ.re p = 0 := by
    simpa [ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicResidue]
      using pairAgreementDefect_zero_every_prime_of_inverse_prime_harmonic_minimum
        h_min ρ hρ p hp
  exact (ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicObservable_eq_zero_iff_residue_zero
    ρ.re p).mpr hres
...
/-- An off-line zero makes the localized forked-PerC detector harmonic
strictly positive at every prime.  This is the detector-native no-cancellation
obstruction: the Gaussian weight is positive and the two-cosh agreement
residue is positive off the critical line. -/
theorem detectorPrimeHarmonicObservable_pos_every_prime_of_offline_zero
    {ρ : ℂ} (hρ : ρ ∈ ZD.NontrivialZeros) (hoff : ρ.re ≠ 1/2) :
    ∀ p : ℕ, Nat.Prime p →
      0 < ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicObservable
        ρ.re p := by
  intro p hp
  rw [ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicObservable_eq_residue_weight]
  have hres :
      0 < ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicResidue
        ρ.re p := by
    simpa [ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicResidue]
      using pairAgreementDefect_pos_every_prime_of_offline_zero hρ hoff p hp
  have hψ :
      0 < (ψ_gaussian (Real.log (↑p))) ^ 2 :=
    sq_pos_of_ne_zero (ne_of_gt (ψ_gaussian_pos (Real.log (↑p))))
  exact mul_pos hres hψ

/-- The every-zero/every-prime detector-harmonic vanishing target is exactly
as strong as the no-offline-zero conclusion.  This is the closing checkpoint:
proving the left side unconditionally is equivalent to proving the RH-level
real-part statement. -/
theorem detectorPrimeHarmonicObservable_zero_every_prime_iff_no_offline_zeros :
    (∀ ρ : ℂ, ρ ∈ ZD.NontrivialZeros →
      ∀ p : ℕ, Nat.Prime p →
        ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicObservable
          ρ.re p = 0) ↔
    (∀ ρ : ℂ, ρ ∈ ZD.NontrivialZeros → ρ.re = 1/2) := by
  constructor
  · exact no_offline_zeros_final_of_detectorPrimeHarmonicObservable_zero
  · intro hline ρ hρ p hp
    have hres : ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicResidue
        ρ.re p = 0 := by
      simpa [ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicResidue]
        using pairAgreementDefect_zero_every_prime_of_online_zero
          hρ (hline ρ hρ) p hp
    exact (ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicObservable_eq_zero_iff_residue_zero
      ρ.re p).mpr hres
...
/-- Zero defect energy for actual zeta zeros gives the detector-native
every-prime harmonic vanishing statement.  The proof uses no RH input: the
Gaussian positivity theorem first rules out off-line real parts, and then the
pure two-cosh geometry converts the critical-line conclusion into vanishing of
the localized forked-PerC prime harmonic at each Euler-log prime scale. -/
theorem detectorPrimeHarmonicObservable_zero_every_prime_of_weil_cauchy_zero_defect_energy
    (h_energy : WeilCauchyZeroDefectEnergy_target_local) :
    ∀ ρ : ℂ, ρ ∈ ZD.NontrivialZeros →
      ∀ p : ℕ, Nat.Prime p →
        ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicObservable
...
/-- The Cauchy/Weil orthogonality extraction is enough to prove the real
detector-prime vanishing target, without using the RH-equivalent converse. -/
theorem detectorPrimeHarmonicObservable_zero_every_prime_of_cauchy_weil_extraction
    (h_orth :
      ZeroOrthogonality.ZeroCoefficientVanishesByOrthogonality)
    (h_cw : CauchyWeilGaussianDefectExtraction_target_local) :
    ∀ ρ : ℂ, ρ ∈ ZD.NontrivialZeros →
      ∀ p : ℕ, Nat.Prime p →
        ZD.WeilPositivity.FinalAssembly.detectorPrimeHarmonicObservable
          ρ.re p = 0 :=
  detectorPrimeHarmonicObservable_zero_every_prime_of_weil_cauchy_zero_defect_energy
    (WeilCauchyZeroDefectEnergy_of_cauchy_weil_extraction h_orth h_cw)

/-- RH close from the real Cauchy/Weil extraction route.  The only analytic
inputs are the zero-side orthogonality principle and the defect-weighted
Cauchy/Weil vanishing package; no critical-line statement is assumed. -/
theorem rh_final_of_cauchy_weil_extraction
    (h_orth :
      ZeroOrthogonality.ZeroCoefficientVanishesByOrthogonality)
    (h_cw : CauchyWeilGaussianDefectExtraction_target_local) :
    RiemannHypothesis :=
...
#print axioms detectorPrimeHarmonicObservable_zero_every_prime_of_inverse_prime_harmonic_minimum
...
#print axioms detectorPrimeHarmonicObservable_pos_every_prime_of_offline_zero
#print axioms detectorPrimeHarmonicObservable_zero_every_prime_iff_no_offline_zeros
