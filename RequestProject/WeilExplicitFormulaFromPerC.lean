...
import RequestProject.H3Closure
/-!
...
theorem pairTestMellin_eq_sum_coshGaussMellinExt (β : ℝ) {s : ℂ} (hs : -2 < s.re) :
  ...
  have hf_an : AnalyticOnNhd ℂ (Contour.pairTestMellin β) U := by
    intro z (hz : -2 < z.re)
  ...
  have h_diff_eventuallyEq : (Contour.pairTestMellin β - g) =ᶠ[nhds (2 : ℂ)] 0 := by
...
theorem rectangleResidueIdentity_from_perC
  ...
  have h_sum_eq :
    ...
    have h_term : ∀ ρ ∈ Z, (↑(n ρ) : ℂ) * Contour.pairTestMellin β ρ =
  ...
  have h_ptwise_re : ∀ z : ℂ, -2 < z.re →
      Contour.weilIntegrand (Contour.pairTestMellin β) z =
  ...
  have h_integ_eq :
      rectContourIntegral (-1) 2 T (Contour.weilIntegrand (Contour.pairTestMellin β)) =
    ...
    have h_bot : ∀ x ∈ Set.uIcc (-1:ℝ) 2,
        Contour.weilIntegrand (Contour.pairTestMellin β) ((x : ℂ) + (-T : ℝ) * I) =
...
- `h_star  : weil_explicit_formula_cosh_pair_target β` — the `(★)` placeholder
  (`∫arch β 2 = ∫weilIntegrand(pairTestMellin β)(-1+iy)`),
  placeholder (`WeilExplicitFormulaPlaceholder.lean:65`).
- `h_refl_zero : archPair_eq_primePair_at_two_target β` —  `∫reflectedPrimeIntegrand β 2 = 0`.
